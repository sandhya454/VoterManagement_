...
const DataTable = () => {
  ...
  const [Filters, setFilters] = useState(() => {
    ...
    return storedFilters ? JSON.parse(storedFilters) : { Wards:49 };
  });
...
const getCount=async()=>{
  try{
    const response = await fetch('https://admin-api.stepnext.com/admin/get-counts')
...
useEffect(()=>{
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`https://admin-api.stepnext.com/admin/get-all-data/${Object.keys(Filters)[0]}/${Filters[Object.keys(Filters)[0]]}`);
